...
eggs_sum <- eggs_data %>%
  ...
  summarise(Egg_count = max(Egg_num)) |> # decided to use max num of eggs, but could have counted the number of rows as well  
  ungroup()
...
nest_species_avg <- nest_species_avg |> 
  left_join(species_data, join_by(Species == Code))  # Diff names, same values   select
...
snowcover_data_fixed2 <- snowcover_data_fixed %>%
  mutate(Date2 = ifelse(Date == "8&9 june 06", as_date(8-June-06), as_date(Date)))
```
